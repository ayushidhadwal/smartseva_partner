...
const AcceptedDeclineRequestScreen = ({navigation}) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const dispatch = useDispatch();
  const {get_decline_accepted_booking_list} = useSelector(
    state => state.request,
  );
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', async () => {
      ...
      setError(null);
      try {
        await dispatch(requestAction.get_accepted_decline_list());
      } catch (e) {
        setError(e.message);
      }
      setLoading(false);
  ...
  return (
    <View style={styles.screen}>
      ...
      />
      {get_decline_accepted_booking_list.length === 0 ? (
        <Text style={styles.activity}>Data Not Found!</Text>
      ) : (
        <FlatList
          showsVerticalScrollIndicator={false}
          data={get_decline_accepted_booking_list}
...
export default AcceptedDeclineRequestScreen;
